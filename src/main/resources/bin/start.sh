...
do_start(){
	...
	if [ ! -n "$ENV" ];then
	   nohup java $JAVA_OPTIONS -jar $JAR_NAME --spring.profiles.active=$PROFILE >/dev/null 2>&1 &
	else
	   nohup $JAVA_HOME/bin/java $JAVA_OPTIONS -jar $JAR_NAME --spring.profiles.active=$PROFILE >/dev/null 2>&1 &
	fi
